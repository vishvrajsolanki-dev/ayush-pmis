...
```text
Authority 1 (technical): ANCHOR_MASTER_DESIGN.md
Authority 2 (narrative):  ANCHOR_LATEST_IDEA.md
...
ID: F-01
Severity: P2
Document(s): ANCHOR_TRACEABILITY.md
...
specified in ANCHOR_AI_DS_SPEC.md (§5, Cycle-T rule) and IS tested (LT-01 in
ANCHOR_TEST_PLAN.md), so this is a traceability-matrix gap, not a functional
...
Exact correction required: Add one row to ANCHOR_TRACEABILITY.md's
...
ID: F-02
Severity: P2
Document(s): ANCHOR_ARCHITECTURE.md, ANCHOR_IMPLEMENTATION_PLAN.md (no document is the clear owner)
...
ANCHOR_STATE_MACHINES.md §7) with partial detail. allocation_run and
recovery_processing appear only as row labels in ANCHOR_TRACEABILITY.md.
...
ANCHOR_ARCHITECTURE.md (§9, currently a short paragraph) reproducing all 9
...
Exact correction required: In ANCHOR_UX_SPEC.md §2, change
...
- F-01 (FR-009 traceability row) — five-minute fix, ANCHOR_TRACEABILITY.md.
- F-02 (job inventory) — the background-job workstream (Milestone 7/8/11 in ANCHOR_IMPLEMENTATION_PLAN.md) should not start without this table in hand; recommend fixing before that workstream, not before the whole implementation effort.
...
- Deletion/pseudonymization endpoint's exact path (Master gap, correctly flagged in ANCHOR_API_SPEC.md and ANCHOR_TRACEABILITY.md as Deferred rather than invented).
- Admin deactivation trigger/audit-event name for Institution/Company accounts (Master gap, correctly flagged "(not enumerated in Master)" in ANCHOR_STATE_MACHINES.md).

### Optional documentation polish
- F-03 (UX self-reference numbering).
- ANCHOR_SRS.md's "Master Ref" column mixes "§N" citations inherited from the Master's own FR table (which itself uses an older internal numbering scheme not fully aligned with this Master's "Part N" headers) alongside directly-added "Part 6.1/6.2"-style references. This ambiguity originates in the Master document itself, not in the SRS — no downstream correction is owed unless the Master's own FR table is revisited, which is out of scope for this audit ("do not repair the Master").
- ADR-01 (candidate-proposing DA) does not itself restate the "never extends to recovery/overrides" caveat inline — this is accurate as written (it makes no false claim), and the caveat is stated clearly and repeatedly elsewhere (ANCHOR_ALLOCATION_ENGINE.md §5, ANCHOR_PRD.md, ANCHOR_SRS.md, ANCHOR_STATE_MACHINES.md, ANCHOR_UX_SPEC.md). Adding a one-line cross-reference in ADR-01 would be a nice-to-have, not a correction.
...
|---|---|---|
| Module boundaries, service topology (single FastAPI service, in-process/scheduled-worker APScheduler) | ANCHOR_ARCHITECTURE.md | Harmless derived engineering structure — explicitly labeled **[Derived]** |
| Path-naming convention (`/system/`, `/admin/` prefixes) | ANCHOR_API_SPEC.md | Harmless derived engineering structure — explicitly labeled Derived, stated as revisable without a Master update |
| Entity/field-level schema (tables, columns, indexes) | ANCHOR_DATA_MODEL.md | Harmless derived engineering structure — explicitly labeled **[Derived]**, required to make the Master's locked rules implementable |
| Endpoint inventory (paths, methods, request/response shapes) | ANCHOR_API_SPEC.md | Harmless derived engineering structure — explicitly labeled Derived |
| Milestone numbering/dependency graph | ANCHOR_IMPLEMENTATION_PLAN.md | Harmless derived engineering structure — no calendar dates invented, dependency rationale preserved from Master Part 31 |
| Test IDs (UT/CT/TT/LT/AT/ST/SEC/STT/RT/E2E/DT/FB) | ANCHOR_TEST_PLAN.md | Harmless derived engineering structure — required to make the Master's test strategy executable |
...
|---|---|---|---|
| E/F separation | CT-01, UT-01, UT-02 | Yes | ANCHOR_TEST_PLAN.md §3, §2 |
...
| **Cycle-T label reservation (FR-009)** | LT-01 | Yes, functionally — **not listed by FR ID in ANCHOR_TRACEABILITY.md (F-01)** | §5 |
...
- **Frontend team:** Can build from ANCHOR_UX_SPEC.md + ANCHOR_API_SPEC.md + ANCHOR_STATE_MACHINES.md. Ready, modulo the trivial F-03 fix.
- **Backend team (request-driven endpoints):** Can build from ANCHOR_API_SPEC.md + ANCHOR_SECURITY_PRIVACY.md + ANCHOR_DATA_MODEL.md. Ready.
...
- **Database owner:** Can build the schema directly from ANCHOR_DATA_MODEL.md. Ready.
- **ML engineer:** Can implement the `O` pipeline directly from ANCHOR_AI_DS_SPEC.md (feature contracts, temporal partitioning, activation gate, constants, pseudocode-level activation logic all present). Ready.
- **Allocation engineer:** Can implement DA/recovery directly from ANCHOR_ALLOCATION_ENGINE.md, including runnable pseudocode for all 9 sub-algorithms. Ready.
- **QA:** Can write tests without guessing, using ANCHOR_TEST_PLAN.md's ID scheme — except the one FR-009 gap (F-01), which would surface as a five-minute clarifying question, not a blocker.
- **Security/privacy implementer:** Can implement without guessing from ANCHOR_SECURITY_PRIVACY.md; every non-claim and every state transition affecting privacy is present.
...
1. Add the missing FR-009 row to ANCHOR_TRACEABILITY.md (F-01).
2. Add a full 9-job inventory table (trigger/prerequisites/input/output/retry/idempotency/failure) to ANCHOR_ARCHITECTURE.md §9, sourced verbatim from Master Part 19 — no new jobs, no invented numbers (F-02).
3. Correct the "(Screen 12)" cross-reference in ANCHOR_UX_SPEC.md §2 to "(Screen 11, Override Experience)" (F-03).
